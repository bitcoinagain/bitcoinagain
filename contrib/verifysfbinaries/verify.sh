...
###   This script attempts to download the signature file SHA256SUMS.asc from bitcoinagain.org
...
WORKINGDIR="/tmp/bitcoinagain"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://bitcoinagain.org/bin/"
VERSIONPREFIX="bitcoinagain-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinagain-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bitcoinagain Core binary release signing key installed?"
   fi
