...
*Setup instructions for a Gitian build of Bitcoinagain using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Bitcoinagain
...
- [Building Bitcoinagain](#building-bitcoinagain)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-bitcoinagain-host/README.md).
...
Clone the git repositories for bitcoinagain and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bitcoinagain/bitcoinagain
```
...
in the bitcoinagain repository under 'Fetch and build inputs' to install sources which require
...
To build Bitcoinagain (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bitcoinagain repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitcoinagain/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bitcoinagain/bitcoinagain
...
For example:
```bash
URL=https://github.com/laanwj/bitcoinagain.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bitcoinagain=${COMMIT} --url bitcoinagain=${URL} ../bitcoinagain/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoinagain=${COMMIT} --url bitcoinagain=${URL} ../bitcoinagain/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitcoinagain=${COMMIT} --url bitcoinagain=${URL} ../bitcoinagain/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bitcoinagain git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitcoinagain/contrib/gitian-descriptors/*|sort|uniq )
...
BTCAPATH=/some/root/path/bitcoinagain.git
SIGPATH=/some/root/path/bitcoinagain-detached-sigs.git

./bin/gbuild --url bitcoinagain=${BTCAPATH},signature=${SIGPATH} ../bitcoinagain/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcoinagain-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcoinagain-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitcoinagain-osx-build.assert
```
...
[bitcoinagain/gitian.sigs](https://github.com/bitcoinagain/gitian.sigs/) repository, or if that's not possible create a pull
